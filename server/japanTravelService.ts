...
async function generatePlan(model: ReturnType<GoogleGenerativeAI['getGenerativeModel']>, payload: any) {
  ...
  const prompt = `
            ...
            **주요 지침:**
            1.  **여행 개요 (tripOverview):** 전체 여행의 컨셉과 주요 활동을 요약하는 3~4문장의 개요를 작성해주세요.
            2.  **예상 비용 (estimatedCost):** 항공권을 제외한 1인 기준 총 예상 경비를 원화(KRW)로 제시해주세요.
            3.  **숙소와 맛집:** 실제 존재하는 장소의 이름으로, 각각 최소 5개 이상 추천해주세요.
            4.  **평점:** 모든 추천 장소에 5점 만점의 평점(rating)을 반드시 포함하고, 평점이 높은 순으로 정렬해주세요.
            5.  **상세 일정:** 하루 일정을 아침, 점심, 저녁 식사를 포함하여 최소 5개 이상의 시간대로 세분화하여 계획해주세요.
            6.  **교통편:** 각 일정(schedule) 항목마다 이동을 위한 구체적인 교통편 정보(예: 지하철 노선, 버스 번호, 도보 등), 예상 소요 시간, 예상 요금을 'transportation' 필드에 상세히 작성해주세요.
            7.  **일정 내용:** 각 활동 및 방문 장소에 대한 구체적인 설명, 예상 소요 시간, 팁 등을 **다섯 문장 이상**으로 아주 상세하게 작성해주세요.
            8.  **동선:** 논리적이고 효율적인 동선으로 일정을 계획하고, 모든 필수 방문 장소를 포함해야 합니다.
            9.  **출력 형식:** 결과는 다른 설명이나 markdown 포맷 없이, 아래와 같은 순수한 JSON 형식으로만 반환해주세요.

            **JSON 출력 형식 예시:**
            {
              "tripTitle": "오사카 미식과 교토의 고즈넉함을 담은 3박 4일",
              "tripOverview": "이 3박 4일 여행은 오사카의 활기찬 미식 문화와 교토의 고즈넉한 역사적 명소를 모두 경험할 수 있도록 설계되었습니다. 주요 관광지를 효율적으로 둘러보고, 현지 음식을 맛보며, 편안한 숙소에서 휴식을 취하는 것을 목표로 합니다.",
