...
async function generatePlan(model: ReturnType<GoogleGenerativeAI['getGenerativeModel']>, payload: any) {
  ...
  const transportationInstructions = travelCountry === Country.KOREA 
    ? `서울에서 ${destination}까지 다음과 같은 교통수단을 고려하여 계획을 세워주세요:
       1. KTX/SRT: 서울역, 용산역 또는 수서역에서 출발
       2. 고속/시외버스: 서울고속버스터미널이나 동서울터미널에서 출발
       3. 자가용: 네비게이션 기준 예상 소요시간과 권장 경로, 휴게소 정보 포함
       4. 국내선 항공편: 김포공항에서 출발 (필요한 경우)
       해당 교통수단들의 장단점, 소요시간, 비용을 비교하여 가장 효율적인 이동방법을 추천해주세요.`
    : `인천 국제공항에서 출발하는 항공편을 기준으로 계획을 세워주세요.`;

  const prompt = `
            당신은 ${travelCountry} 전문 여행 플래너입니다. ${startDate}부터 ${endDate}까지 서울에서 ${travelCountry}의 ${destination}으로 떠나는 여행을 위한 상세한 계획을 세워주세요.
            ${mustVisitText}
            ${hotelInstruction}
            ...
            **주요 지침:**
            ...
            7.  **현실적인 교통편 연계:** ${travelCountry === Country.KOREA 
              ...
              : '인천공항 출발/도착 시간을 현실적으로 고려해주세요. (예: 출발 오전 9시 이후, 도착 저녁 8시 이전)'}
